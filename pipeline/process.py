...
Run on evo (where the docker images and model weights live).
"""
import argparse
import json
import shutil
...
def resolve_input(args, out_dir: Path) -> tuple[Path, dict]:
    """Either download from URL or copy the local file. Returns (audio_path, metadata)."""
    if args.url:
        print(f"[pipeline] yt-dlp ← {args.url}", file=sys.stderr)
        audio, meta = yt_mod.download(args.url, out_dir)
        return audio, meta
...
def main() -> int:
    ...
    if not vocals:
        raise SystemExit("no vocals stem produced; aborting")

    # 3. ASR on vocals
    t = time.perf_counter()
    lyrics_path = out_dir / "lyrics.json"
    asr_mod.transcribe(vocals, lyrics_path, language=args.language, code_dir=ASR_CODE_DIR)
    ...
    # 4. LRCLib lookup
    artist = args.artist or meta.get("uploader") or meta.get("channel") or ""
